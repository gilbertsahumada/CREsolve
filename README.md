...
2. `yarn e2e:setup` — Deploys contracts, creates 3 demo markets, registers workers
3. `yarn e2e:test` — Runs 12 integration tests via Vitest
...
12 integration tests covering agent health, market resolution workflows, reputation scoring, and edge cases. Timeout: 120s per test.
...
yarn demo
yarn demo 1   # specific market
yarn demo 2
```

`setup-demo.ts` deploys `CREsolverMarket`, creates 3 markets, funds workers, and writes `demo-config.json`.
`demo-run.ts` reads the config, queries agents, and executes the resolution workflow.
...
│   ├── setup-demo.ts            # Deploy + create markets
│   ├── demo-run.ts              # Execute resolution
│   └── demo-config.json         # Generated by setup
│
├── e2e/                         # E2E test suite
│   ├── vitest.e2e.config.ts
│   ├── setup.ts                 # Anvil + agent bootstrap
│   ├── e2e.test.ts              # 12 integration tests
...
Generated by `yarn setup`. Contains RPC URL, deployed contract address, worker wallets (Anvil HD keys), and market count. Do not commit — it is gitignored.
...
4. **EVALUATE** — Score workers across 5 dimensions, compute consensus
