...
yarn sepolia:register    # legacy alias -> sepolia:sync
yarn sepolia:metadata    # legacy alias -> sepolia:normalize
...
# (legacy alias: yarn sepolia:metadata)
...
│   ├── register-agents.ts       # Compatibility wrapper -> sync-agents.ts (--mode full)
│   ├── update-agent-metadata.ts # Compatibility wrapper -> sync-agents.ts (--mode normalize)
