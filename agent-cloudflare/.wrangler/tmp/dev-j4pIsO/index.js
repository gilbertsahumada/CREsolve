...
// src/validation.ts
var resolveRequestSchema = external_exports.object({
  market_id: external_exports.number().int().min(0),
  question: external_exports.string().min(1),
  deadline: external_exports.number().optional(),
  context: external_exports.string().optional()
});
var challengeRequestSchema = external_exports.object({
  challenges: external_exports.array(external_exports.string().min(1)).min(1)
});
...
function setCache(marketId, response) {
...
__name(hashQuestion, "hashQuestion");
function mockInvestigate(question) {
  const q = question.toLowerCase();
  if (q.includes("bitcoin") && q.includes("200k")) {
    return {
      determination: false,
      confidence: 0.65,
      evidence: "Bitcoin reaching $200k requires sustained institutional adoption and favorable macro conditions. Current market dynamics and historical volatility patterns suggest this target is unlikely within the specified timeframe.",
      sources: [
  ...
  if (q.includes("ethereum") && q.includes("pos")) {
    return {
      determination: true,
      confidence: 0.99,
      evidence: "Ethereum successfully transitioned to Proof of Stake via The Merge on September 15, 2022. The beacon chain has been running stably with over 900k validators. This is a verified historical fact.",
      sources: [
  ...
  if (q.includes("etf")) {
    return {
      determination: true,
      confidence: 0.72,
      evidence: "Multiple spot Bitcoin ETFs were approved by the SEC in January 2024. ETF approval has been a significant catalyst for institutional adoption and price appreciation.",
...
__name(mockInvestigate, "mockInvestigate");
var MAX_RETRIES = 3;
var BASE_DELAY_MS = 1e3;
async function llmInvestigate(question, config) {
  const openai = new openai_default({ apiKey: config.llmApiKey });
...
__name(llmInvestigate, "llmInvestigate");
async function investigate(question, marketId, config) {
  if (marketId !== void 0) {
    const cached = getCached(marketId);
    if (cached) return cached;
  }
  const result = config.isLlmMode ? await llmInvestigate(question, config) : mockInvestigate(question);
  if (marketId !== void 0) {
...
function hashChallenges(challenges) {
  return challenges.join("\0");
}
__name(hashChallenges, "hashChallenges");
function getCached2(challenges) {
...
__name(getCached2, "getCached");
function setCache2(challenges, responses) {
  const key = hashChallenges(challenges);
  responseCache2.set(key, { responses, timestamp: Date.now() });
}
__name(setCache2, "setCache");
function mockDefend(challenges) {
...
__name(mockDefend, "mockDefend");
var MAX_RETRIES2 = 3;
var BASE_DELAY_MS2 = 1e3;
async function llmDefend(challenges, config) {
  const openai = new openai_default({ apiKey: config.llmApiKey });
...
${challenges.map((challenge, index) => `${index + 1}. ${challenge}`).join("\n")}`
      ...
      if (!content) throw new Error("Empty LLM response");
      const parsed = JSON.parse(content);
      return Array.isArray(parsed.responses) ? parsed.responses.map(String) : challenges.map(() => "Unable to parse defense response.");
    } catch (err) {
      if (attempt === MAX_RETRIES2) throw err;
      const delay = BASE_DELAY_MS2 * Math.pow(2, attempt - 1);
      await new Promise((resolve) => setTimeout(resolve, delay));
    }
  }
  throw new Error("Unreachable");
}
__name(llmDefend, "llmDefend");
async function defend(challenges, config) {
  const cached = getCached2(challenges);
  if (cached) return cached;
  const responses = config.isLlmMode ? await llmDefend(challenges, config) : mockDefend(challenges);
  setCache2(challenges, responses);
  return responses;
}
__name(defend, "defend");

// src/routes/a2a.ts
var a2aRoutes = new Hono2();
...
// .wrangler/tmp/bundle-QEnRgr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QEnRgr/middleware-loader.entry.ts
