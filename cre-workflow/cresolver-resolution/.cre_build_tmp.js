...
var EvmConfigSchema = exports_external.object({
  chain_selector: exports_external.union([exports_external.number().int().nonnegative(), exports_external.string().regex(/^\d+$/)]),
  market_address: exports_external.string(),
  receiver_address: exports_external.string(),
  gas_limit: exports_external.number()
});
...
var ConfigSchema3 = exports_external.object({
  ...
  agents: exports_external.array(AgentConfigSchema).min(1)
});
var getMarketAbi = [
  {
    name: "getMarket",
...
function readMarketWorkers(runtime2, evmClient, marketId) {
  const config = runtime2.config;
  const evm = config.evms[0];
  const marketAddr = evm.market_address;
  ...
  const workersResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: ZERO_ADDRESS,
      to: marketAddr,
  ...
  const workers = [];
  for (let i2 = 0;i2 < workerAddresses.length; i2++) {
    const addr = workerAddresses[i2];
    const agent = config.agents[i2];
    if (!agent)
      continue;
    ...
    const stakeResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: ZERO_ADDRESS,
        to: marketAddr,
        data: stakeCallData
    ...
    const repResult = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: ZERO_ADDRESS,
        to: marketAddr,
        data: repCallData
  ...
  runtime2.log(`Read ${workers.length} workers for market ${marketId}: "${market.question.slice(0, 60)}..."`);
  return workers;
}
function readMarketQuestion(runtime2, evmClient, marketId) {
  const marketAddr = runtime2.config.evms[0].market_address;
...
function submitResolution(runtime2, evmClient, marketId, result) {
  const evm = runtime2.config.evms[0];
  const receiverAddr = evm.receiver_address;
  ...
  runtime2.log(`Resolution submitted for market ${marketId}: ${result.resolution ? "YES" : "NO"} with ${result.workers.length} workers`);
}
function clamp(value2, min = 0, max = 100) {
  return Math.max(min, Math.min(max, Math.round(value2)));
}
function scoreResolutionQuality(det) {
  let score = 40;
...
function scoreSourceQuality(det) {
  let score = 30;
  score += Math.min(30, det.sources.length * 10);
  const domains = new Set(det.sources.map((s) => {
    try {
...
function scoreAnalysisDepth(det, challengeResult) {
  let score = 30;
  score += Math.min(25, Math.floor(det.evidence.length / 25));
  if (challengeResult && challengeResult.responses.length > 0) {
    const avgResponseLen = challengeResult.responses.reduce((sum, r) => sum + r.length, 0) / challengeResult.responses.length;
...
function evaluateWorkers(determinations, challengeResults) {
...
function computeResolution(determinations, evaluations, workers) {
  ...
  for (const det of determinations) {
    ...
    if (!ev || !w)
      continue;
    const repFactor = w.reputation.count > 0 ? (w.reputation.resQuality + w.reputation.srcQuality + w.reputation.analysisDepth) / 3 / 100 + 0.5 : 1;
...
function generateChallenges(determination, allDeterminations) {
  const hasDisagreement = allDeterminations.some((d) => d.determination !== determination.determination);
  if (hasDisagreement) {
    return [
...
function queryAllAgents(runtime2, workers, marketId, question) {
  ...
  for (const worker of workers) {
    try {
      ...
      const queryFn = runtime2.runInNodeMode((nodeRuntime) => {
      ...
      if (result) {
        ...
        runtime2.log(`Worker ${worker.address.slice(0, 10)}...: ${result.determination ? "YES" : "NO"} (${(result.confidence * 100).toFixed(0)}%)`);
      }
    } catch {
      runtime2.log(`Worker ${worker.address.slice(0, 10)}... failed to respond`);
    }
  }
  ...
  if (determinations.length !== workers.length) {
    const responded = new Set(determinations.map((d) => d.workerAddress.toLowerCase()));
    const missing = workers.filter((w) => !responded.has(w.address.toLowerCase())).map((w) => w.address);
    throw new Error(`Missing determinations from ${missing.length} worker(s): ${missing.join(", ")}`);
  }
  runtime2.log(`${determinations.length}/${workers.length} workers responded`);
  return determinations;
}
function challengeAllAgents(runtime2, workers, determinations) {
  const workerMap = new Map(workers.map((w) => [w.address, w]));
  ...
  for (const det of determinations) {
    ...
    try {
      ...
      const challengeFn = runtime2.runInNodeMode((nodeRuntime) => {
        const response = httpClient.sendRequest(nodeRuntime, {
      ...
      runtime2.log(`${worker.address.slice(0, 10)}... defended ${result.responses.length} challenges`);
    } catch {
      challengeResults.push({
        workerAddress: det.workerAddress,
        challenges,
...
var RESOLUTION_REQUESTED_TOPIC = keccak256(toBytes("ResolutionRequested(uint256,string)"));
function parseChainSelector(value2) {
  return BigInt(typeof value2 === "string" ? value2 : value2);
}
function resolveMarket(runtime2, marketId) {
  const evm = runtime2.config.evms[0];
  const evmClient = new ClientCapability(parseChainSelector(evm.chain_selector));
  runtime2.log(`Starting resolution for market ${marketId}`);
  const question = readMarketQuestion(runtime2, evmClient, marketId);
  const workers = readMarketWorkers(runtime2, evmClient, marketId);
  const determinations = queryAllAgents(runtime2, workers, marketId, question);
  const challengeResults = challengeAllAgents(runtime2, workers, determinations);
  ...
  submitResolution(runtime2, evmClient, marketId, resolution);
  runtime2.log(`Market ${marketId} resolved: ${resolution.resolution ? "YES" : "NO"} with ${resolution.workers.length} workers`);
}
function initWorkflow(config) {
  ...
  const evmLogTrigger = evmClient.logTrigger({
    ...
    topics: [
      { values: [RESOLUTION_REQUESTED_TOPIC] }
    ]
  });
  const handleEvmLogTrigger = (runtime2, log) => {
    const marketIdTopic = log.topics[1];
    const marketId = marketIdTopic ? Number(bytesToBigint(marketIdTopic)) : 0;
    runtime2.log(`EVM Log Trigger: ResolutionRequested for market ${marketId}`);
    resolveMarket(runtime2, marketId);
    return {};
  };
  const httpTrigger = new HTTPCapability().trigger({});
  const handleHttpTrigger = (runtime2, payload) => {
    const inputStr = new TextDecoder().decode(payload.input);
    const input = JSON.parse(inputStr);
    const marketId = input.market_id;
    runtime2.log(`HTTP Trigger: Resolution requested for market ${marketId}`);
    resolveMarket(runtime2, marketId);
    return {};
  };
  return [
