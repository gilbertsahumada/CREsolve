...
| J | E2E Docker Sandbox | ✅ | 12 tests, docker-compose + vitest |
...
- **Report encoding**: CRE workflow encodes `abi.encode(marketId, workers[], weights[], dimScores[], resolution)` — Option 4 blinded (no determinations on-chain)
...
contract CREsolverMarket is Ownable, ReentrancyGuard {
    ...
    function getScoringCriteria() external pure returns (string[8] memory, uint256[8] memory);
}
```
...
```json
{
  "scripts": {
    ...
    "e2e:setup": "cd e2e && npx tsx setup.ts",
