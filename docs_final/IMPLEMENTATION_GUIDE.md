...
| I | Scripts | ✅ | setup-demo.ts, sync-agents.ts, audit-agents-onchain.ts |
...
│   ├── sync-agents.ts                  # Canonical ERC-8004 register/normalize
│   ├── audit-agents-onchain.ts         # Strict on-chain audit
...
│   └── (no local runner)               # Uses CREReceiver report path
├── shared/
...
### E.4 Receiver Path Simulation (e2e.test.ts)

E2E resolves markets through the receiver contract path:
1. Reads market data from chain
2. Queries agents via HTTP (`/a2a/resolve`)
3. Sends challenge prompts (`/a2a/challenge`)
4. Computes resolution inputs (workers, weights, dimScores)
5. Encodes report payload
6. Calls `CREReceiver.onReport(metadata, report)`
...
### F.2 E2E Receiver Resolution

Resolution execution for local simulation is performed from `e2e/e2e.test.ts`:
1. Reads `e2e/demo-config.json`
2. Checks agent health and mode
3. Queries each worker (`POST /a2a/resolve`)
4. Sends challenge prompts (`POST /a2a/challenge`)
5. Computes inputs and encodes report payload
6. Calls `CREReceiver.onReport(...)`
7. Verifies on-chain resolution and reputation updates
