...
async function main() {
  ...
  // Use Anvil account[0] as deployer (standard HD wallet)
  ...
  const tx0 = await (contract as ethers.Contract).setAuthorizedResolver(
    deployer.address,
    true,
  );
  await tx0.wait();
  ...
  for (let m = 0; m < DEMO_MARKETS.length; m++) {
    ...
    const createTx = await (contract as ethers.Contract).createMarket(
      market.question,
  ...
  const demoConfig = {
    rpcUrl: RPC_URL,
    contractAddress,
    resolverPrivateKey: deployer.privateKey,
